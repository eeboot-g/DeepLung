...
pdframe =  pd.read_csv('annotationdetclsconvfnl_v3_O.csv', names=['seriesuid', 'coordX', 'coordY', 'coordZ', 'diameter_mm', 'malignant'])
...
# fid.close()
print('newlst',len(newlst))
preprocesspath = '/home/zhaojie/zhaojie/Lung/data/luna16/cls/train6-9/'
savepath = '/home/zhaojie/zhaojie/Lung/data/luna16/cls/crop_v3/'
import os
import os.path 
...
for idx in range(len(newlst)):
	...
	# print(os.path.join(preprocesspath, pid+'_clean.npy'))
	...
	cropdata = np.ones((CROPSIZE, CROPSIZE, CROPSIZE))*170#(17, 17, 17)
	cropdatatmp = np.array(data[0, bgx:bgx+CROPSIZE, bgy:bgy+CROPSIZE, bgz:bgz+CROPSIZE])#(17, 17, 17)
	# print('cropdatatmp0.shape',np.min(cropdatatmp),np.max(cropdatatmp))
	# print('cropdatatmp1.shape',np.min(cropdatatmp),np.max(cropdatatmp))
	...
	# print('cropdata.shape',np.min(2-cropdatatmp),np.max(2-cropdatatmp))
